...
class HusThemeFunctions(QObject):
    ...
    @Slot(QColor, bool, QColor, result = list)
    @staticmethod
    def genColorString(color: QColor,
                       light: bool = True,
                       background: QColor = QColor()) -> List[str]:
    ...
    def onBackground(color: QColor, background: QColor) -> QColor:
        ...
        return QColor.fromRgbF(
            (fg.redF() * fg.alphaF() + bg.redF() * bg.alphaF() *
             (1 - fg.alphaF())) / alpha_val,
            (fg.greenF() * fg.alphaF() + bg.greenF() * bg.alphaF() *
             (1 - fg.alphaF())) / alpha_val,
            (fg.blueF() * fg.alphaF() + bg.blueF() * bg.alphaF() *
             (1 - fg.alphaF())) / alpha_val, alpha_val)
